...
void ngli_mat4_skew(float *dst, const float *skew, const float *axis)
{
    memset(dst, 0, 4 * 4 * sizeof(*dst));
    dst[ 0] = 1.f;
    dst[ 1] = axis[1] * skew[0];  // X skew on Y-axis
    dst[ 2] = axis[2] * skew[0];  // X skew on Z-axis
    dst[ 4] = axis[0] * skew[1];  // Y skew on X-axis
    dst[ 5] = 1.f;
    dst[ 6] = axis[2] * skew[1];  // Y skew on Z-axis
    dst[ 8] = axis[0] * skew[2];  // Z skew on X-axis
    dst[ 9] = axis[1] * skew[2];  // Z skew on Y-axis
    dst[10] = 1.f;
    dst[15] = 1.f;
}
